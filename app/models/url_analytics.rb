module TrafficSpy
  class UrlAnalytics
    ...
    def longest_response_time(identifier)
      ...
      client.urls.maximum(:responded_in)
    end

    def shortest_response_time(identifier)
      client = find_client(identifier)
      client.urls.minimum(:responded_in)
    end

    def average_response_time(identifier)
      client = find_client(identifier)
      client.urls.average(:responded_in).round(3).to_f
    end

    def verbs_used(identifier)
      client = find_client(identifier)
      client.urls.pluck(:request_type)
    end

    def most_popular_referrers(identifier)
      client = find_client(identifier)
      referrers = client.urls.group(:referred_by).count
